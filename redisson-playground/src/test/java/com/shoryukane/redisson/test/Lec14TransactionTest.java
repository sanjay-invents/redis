...
public class Lec14TransactionTest extends BaseTest {
    ...
    public void transactionTest() {
        RTransactionReactive transactionReactive = this.client.createTransaction(TransactionOptions.defaults());
        RBucketReactive<Long> user1Balance = transactionReactive.getBucket("user:1:balance", LongCodec.INSTANCE);
        RBucketReactive<Long> user2Balance = transactionReactive.getBucket("user:2:balance", LongCodec.INSTANCE);
        this.transfer(user1Balance, user2Balance, 50)
                .thenReturn(0)
                // .map(i -> (5 / i)) // Some error
                .then(transactionReactive.commit())
                .doOnError(System.out::println)
                .onErrorResume(ex -> transactionReactive.rollback())
                .subscribe();
